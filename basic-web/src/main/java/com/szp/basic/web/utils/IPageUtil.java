package com.szp.basic.web.utils;
...
public class IPageUtil {
    ...
    public static <T, S> IPage<T> supperTransfer(Supplier<IPage<S>> source, Function<S, T> converter) {
        IPage<S> iPage = source.get();
        if (Objects.isNull(iPage)) {
            iPage = (Page<S>)EMPTY_PAGE;
        }
